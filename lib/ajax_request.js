...
function uploadPositions(sub_id) {
	...
	while(positions.length >= 1000) {
		uni.request({
			...
			data: {transport_sub_id: sub_id, position_list: positions.splice(0,1000), current_version: ls.get('app_install_version')},
			method: 'POST',
			dataType: 'json',
			responseType: 'text',
			success: function(res) {
	...
	uni.request({
		...
		url: base_url + 'car/location/saveBatch',
		data: {transport_sub_id: sub_id, position_list: positions, current_version: ls.get('app_install_version')},
		method: 'POST',
		dataType: 'json',
		responseType: 'text',
		success: function(res) {
