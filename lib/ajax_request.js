...
function watchGeoPositionAndSave(sub_id, geocode=false) {
	...
	var wid = plus.geolocation.watchPosition((p) => {
		...
		if (positions) {
			...
			if ((lastPosition.coords.latitude != p.coords.latitude && lastPosition.coords.longitude != p.coords.longitude) || (p.timestamp-lastPosition.timestamp)>=295000) {
				positions.push(p)
			}
		} else {
			positions = [p]
		...
		if (positions.length >= 1 && (p.timestamp-lastPosition.timestamp) >= 1000*userInfo.upload_positions_limit_time) {
