...
function watchGeoPositionAndSave(sub_id, geocode=true, callback = null) {
	...
	var wid = plus.geolocation.watchPosition((p) => {
		...
		if (positions) {
			...
			if ((lastPosition.coords.latitude != p.coords.latitude && lastPosition.coords.longitude != p.coords.longitude) || (p.timestamp-lastPosition.timestamp)>=270000) {
				positions.push(p)
			}
		} else {
			positions = [p]
	...
	}, {geocode: geocode, provider: 'baidu', coordsType: 'bd09ll', maximumAge: 1000*userInfo.watch_position_limit_time})
